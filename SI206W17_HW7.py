...
def get_user_tweets(input_string):
	...
	if input_string in CACHE_DICTION:
		...
		search_results = api.user_timeline(input_string)  ##Returns the 20 most recent statuses posted from the authenticating user or the user specified. Returns a list of Status objects!
	...
	return returnedList  #returns data for 20 of the user's tweets
...
for i in umsi_tweets:
	tweetidlist.append(i['id'])
	authorlist.append(i['user']['screen_name'])
...
listoftups = list(zip(tweetidlist, authorlist, timepostedlist, tweettextlist, retweetlist))
...
x = list(cur.execute(query))
unpacked = [item[0] for item in x]
first_rt = unpacked[0]
